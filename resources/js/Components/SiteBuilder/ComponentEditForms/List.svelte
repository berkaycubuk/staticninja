<script>
    ...
    function removeItem(e, id) {
        ...
        component.list = [...component.list.filter(a => a.id != id)]
...
<div>
    ...
    <form on:change={formChanged} class="p-8 flex flex-col gap-6 overflow-y-auto" style="max-height: calc(100vh - 177px);">
        ...
        <div class="flex flex-col gap-2">
            ...
            {#each component.list as item}
            <div class="p-4 flex flex-col gap-2 bg-neutral-100 rounded">
                <div>
                    <input type="text" class="input w-full text-sm" placeholder="Text" bind:value={item.text} />
                </div>
                <div>
            ...
            <div>
                <button on:click={addItem} class="py-1 px-4 text-sm rounded bg-yellow-200 hover:bg-yellow-300 transition-all">Add button</button>
            </div>
        </div>

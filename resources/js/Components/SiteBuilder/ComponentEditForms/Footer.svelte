<script>
    ...
    function removeLink(e, id) {
        e.preventDefault()

        component.links = [...component.links.filter(a => a.id != id)]
...
<div>
    ...
    <form on:change={formChanged} class="p-8 flex flex-col gap-6 overflow-y-auto" style="max-height: calc(100vh - 177px);">
        ...
        <div class="flex flex-col gap-2">
            <label for="component-title">Copyright</label>
            <input id="component-title" type="text" class="input" placeholder="Title" bind:value={component.section.copyright} />
        </div>
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="p-4 flex flex-col gap-2 bg-neutral-100 rounded">
                <div>
                    <IconInput bind:value={link.icon} onChange={() => updateComponent()} />
                </div>
                <div>
                    <input type="text" class="input w-full text-sm" placeholder="Text" bind:value={link.text} />
                </div>
                <div>
                    <input type="text" class="input w-full text-sm" placeholder="Link" bind:value={link.href} />
                </div>
                <div>
                    <label class="flex items-center gap-2 text-sm text-neutral-800 w-fit cursor-pointer"><input type="checkbox" bind:checked={link.newTab} /> Open in new tab</label>
                </div>
                <div>
                    <button on:click={(e) => removeLink(e, link.id)} class="py-1 px-4 text-sm rounded bg-red-500 hover:bg-red-600 text-white transition-all">Remove</button>
                </div>
            </div>
            {/each}

            <div>
                <button on:click={addLink} class="py-1 px-4 text-sm rounded bg-yellow-200 hover:bg-yellow-300 transition-all">Add link</button>
            </div>
        </div>
